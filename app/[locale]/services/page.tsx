...
const checkIcon = (
  <svg
    ...
    strokeLinecap="round"
    strokeLinejoin="round"
    className="mt-1 h-3.5 w-3.5 shrink-0 text-sky-600"
    aria-hidden="true"
  >
  ...
  return (
    <section className="max-container">
      ...
      <h1 className="head-text">
        {t("services_heading")}{" "}
        <span className="editorial-accent">{t("services_heading_span")}</span>
      </h1>
      <p className="mt-8 editorial-intro">{t("services_intro")}</p>

      <div className="mt-16 grid gap-8 sm:grid-cols-2">
        {services.map((service) => (
          <div
            key={service.key}
            className="editorial-sheet group flex flex-col pb-6 sm:pr-7"
          >
            <div className="block-container w-12 h-12">
              <div className={`btn-back ${service.theme}`} />
              <div className="btn-front flex justify-center items-center text-slate-700">
                {serviceIcons[service.key]}
              </div>
            </div>
            <h2 className="mt-6 text-3xl font-display font-semibold text-ink tracking-[-0.01em]">
        ...
        <ol className="mt-12 grid gap-10 sm:grid-cols-2 lg:grid-cols-4">
          {serviceProcess.map((step, index) => (
            <li key={step.key} className="relative flex flex-col">
              <span className="font-display text-5xl font-semibold text-accent tracking-[-0.02em]">
                {String(index + 1).padStart(2, "0")}
              </span>
              <h4 className="mt-3 font-sans text-base font-medium text-ink">
                {step.title[locale]}
              </h4>
      ...
      <div className="cta">
        ...
        <Link
          href="/contact"
          className="btn"
          aria-label={t("services_cta_button")}
        >
