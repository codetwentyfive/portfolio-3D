...
const CONTACT_API_ENDPOINT = process.env.NEXT_PUBLIC_CONTACT_API_URL || "/api/contact";
...
const ContactContent = () => {
  ...
  const getValidationError = (nextForm: ContactForm) => {
    const now = Date.now();
  ...
  const adjustFoxForScreenSize = (): [Triple, Triple] => {
    // SSR renders the desktop variant; the client re-renders on interaction anyway.
    if (typeof window !== "undefined" && window.innerWidth < 768) {
      return [[0.4, 0.4, 0.4], [0.5, 0.5, 0]];
    }
    if (typeof window !== "undefined" && window.innerWidth < 1024) {
      return [[0.5, 0.5, 0.5], [0.5, 0.35, 0]];
    }
    return [[0.6, 0.6, 0.6], [0.5, 0.2, 0]];
  };
