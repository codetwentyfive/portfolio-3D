...
const SOUNDCLOUD_PLAYLIST: TrackInfo = {
...
const AudioPlayer = () => {
  ...
  const [currentTrack, setCurrentTrack] = useState<TrackInfo>(SOUNDCLOUD_PLAYLIST);
  ...
  const playlistSoundsRef = useRef<SoundCloudSound[]>([]);
  const readyRef = useRef(false);
  const playRequestPendingRef = useRef(false);
  const autoplayRetryTimeoutsRef = useRef<number[]>([]);
  const playbackStateSyncRef = useRef<number | null>(null);
  ...
  const schedulePlaybackStateSync = useCallback((remainingChecks = 12, preservePendingRequest = false): void => {
    clearPlaybackStateSync();
  ...
  const schedulePauseStateSync = useCallback((remainingChecks = 8): void => {
    clearPlaybackStateSync();
  ...
  const seekToMs = useCallback((nextPositionMs: number) => {
    if (!widgetRef.current || !readyRef.current) return;

    const widget = widgetRef.current;
    widget.getDuration((currentDuration) => {
  ...
  const seekByMs = useCallback((offsetMs: number) => {
  ...
  const handlePlayPausePress = useCallback((event: MouseEvent<HTMLButtonElement>) => {
  ...
  const cycleLoop = useCallback(() => {
    setLoopMode((prev) => {
      const modes: LoopMode[] = ['all', 'one', 'off'];
      return modes[(modes.indexOf(prev) + 1) % modes.length];
    });
  }, []);

  const handleSeek = useCallback((event: MouseEvent<HTMLDivElement> | TouchEvent<HTMLDivElement>) => {
  ...
  useEffect(() => {
    ...
    return () => {
      ([
        ...
        'seekto',
        'stop',
      ] as const).forEach((action) => setHandler(action, null));
    };
  }, [
    canControlWidget,
    handleNext,
